        ...
        <>
            <div className="card">
                <div className="card-info">
                    <div className={`card-avatar-${user.gender}`}></div>
                    <div className="card-title">{`${user.firstName} ${user.lastName}`}</div>
                    <div className="card-subtitle">
                        {user.gender === 'male' ? 'male' : 'female'}, {user.age}
                    </div>
                </div>
...
function App() {
    ...
    const filteredUsers = users
        .filter((user) => {
            ...
            if (activeFilter === 'women') return user.gender === 'female';
            if (activeFilter === 'men') return user.gender === 'male';
            return user;
        })
        .sort((a, b) => {
            if (activeFilter === 'name') return a.firstName.localeCompare(b.firstName);
            if (activeFilter === 'age') return a.age - b.age;
            return 0;
        });
        ...
        <>
            <main>
                <div className="container">
                    ...
                    <div className="block__space"></div>
                </div>
            </main>
        </>
    );
